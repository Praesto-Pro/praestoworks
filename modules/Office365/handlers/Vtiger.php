...
class Office365_Vtiger_Handler extends vtigerCRMHandler {
	...
	public function put($recordDetails, $user) {
		...
		$current_user = $user;
		$this->user = $user;
		file_put_contents('logs/sync_debug.log', "Vtiger Handler Put Incoming: " . count($recordDetails['created']) . " created, " . count($recordDetails['updated']) . " updated\n", FILE_APPEND);
		...
		foreach ($createdRecords as $index => $record) {
			...
			try { 
				...
				file_put_contents('logs/sync_debug.log', "Vtiger Handler Create Error: " . $e->getMessage() . "\n", FILE_APPEND);
				$recordDetails['skipped'][] = array('record' => $createdRecords[$index],
													'messageidentifier' => '',
													'message' => $e->getMessage());
				unset($createdRecords[$index]);
		...
		if ($this->isClientUserSyncType()|| $this->isClientUserAndGroupSyncType()) {
			$assignedRecordIds = wsapp_checkIfRecordsAssignToUser($crmIds, $this->user->getId());
			// To check if the record assigned to group
			if($this->isClientUserAndGroupSyncType()){
				$groupIds = $this->getGroupIds($this->user->getId());
		...
		foreach ($updatedRecords as $index => $record) {
			...
			try {
				...
				file_put_contents('logs/sync_debug.log', "Vtiger Handler Update Error: " . $e->getMessage() . "\n", FILE_APPEND);
				$recordDetails['skipped'][] = array('record' => $updatedRecords[$index], 
													'messageidentifier' => '', 
													'message' => $e->getMessage());
				unset($updatedRecords[$index]);
		...
		$assignedDeletedRecordIds = wsapp_checkIfRecordsAssignToUser($deletedCrmIds, $this->user->getId());
		...
		if (!empty($recordDetails['skipped'])) {
			file_put_contents('logs/sync_debug.log', "Vtiger Handler Skipped: " . print_r($recordDetails['skipped'], true) . "\n", FILE_APPEND);
		}
